...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^mdroid_") ; then
        MDROID_BUILD=$(echo -n $1 | sed -e 's/^mdroid_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $MDROID_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        MDROID_BUILD=
    fi
    export MDROID_BUILD
